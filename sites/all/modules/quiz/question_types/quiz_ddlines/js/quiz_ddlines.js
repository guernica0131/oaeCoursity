    ...
    function set_drag_move(dx, dy, x, y, event, set) {
        ...
        if (QuizElementList.engine.isEditMode()) {
            ...
            set.forEach(function(elm) {
                ...
                if ((elm_x < -10 && going_left) || (elm_y < -10 && going_up) || ((elm_x + elm_width) > QuizElementList.engine.getCanvasWidth() + 30 && !going_left) || (elm_y + elm_height) > QuizElementList.engine.getCanvasHeight() + 10 && !going_up) {
                    inside = false;
                    return false;
                }
            });
        ...
        set.forEach(function(elm) {
            if (elm.type === "circle") {
                elm.attr({
                    cx: elm.attr("cx") + (x - elm.set.ox),
                    cy: elm.attr("cy") + (y - elm.set.oy)
                });
            } else {
                elm.attr({
                    x: elm.attr("x") + (x - elm.set.ox),
                    y: elm.attr("y") + (y - elm.set.oy)
                });
            }
        });
        ...
        this.init = function(container_class, node_id) {
            ...
            if (this.isEditMode()) {
                ...
                $('#edit-feedback-enabled').change(function() {
                    ...
                    $('#feedback-disabled').css({
                        'display': (self.feedbackEnabled() ? 'none' : 'block')
                    });
                });
    ...
    function QuizImage(engine, uri, width, height) {
        ...
        if (this.engine.isEditMode()) {
            ...
            if (this.engine.isNew()) {
                this.image.attr({
                    "x": (engine.getCanvasWidth() / 2) - (width / 2),
                    "y": (engine.getCanvasHeight() / 2) - (height / 2)
                });
            }
    ...
    */
    ...
    this.image.resizer.drag(rmove, rstart);*/
        ...
        this.move = function(deltax, deltay) {
            this.image.attr({
                "x": this.image.attr("x") + deltax,
                "y": this.image.attr("y") + deltay
            });
        };
    }
    ...
    function QuizLabel(parent, color, id, text, x, y) {
        ...
        this.setText = function(text) {
            ...
            self.label.attr({
                width: rect_width < QuizUtil.LABEL_MIN_WIDTH ? QuizUtil.LABEL_MIN_WIDTH : rect_width
            });
            self.label.fg.attr({
                width: rect_width < QuizUtil.LABEL_MIN_WIDTH ? QuizUtil.LABEL_MIN_WIDTH : rect_width
            });
            ...
            if (QuizUtil.defined(self.glow)) {
                self.glow.attr({
                    width: rect_width + 8 < QuizUtil.LABEL_MIN_WIDTH + 8 ? QuizUtil.LABEL_MIN_WIDTH + 8 : rect_width + 8
                });
            }
        ...
        this.label.attr({
            ...
            "fill-opacity": 1,
            "stroke-width": 3,
            cursor: (!this.parent.engine.isResultMode() ? "move" : "default")
        });
        this.label.text.attr({
            "font-size": 12,
            cursor: (!this.parent.engine.isResultMode() ? "move" : "default")
        });
        this.label.fg.attr({
        ...
        if (!this.parent.engine.executionModeLines() && this.parent.engine.isResultMode() && QuizUtil.defined(this.parent.uiHotspot)) {
            ...
            if (center != null) {
                this.set.forEach(function(element) {
                    element.attr({
                        x: center.x - (element.attr('width') / 2),
                        y: center.y - (element.attr('height') / 2)
                    });
                });
            }
        }
    ...
    var bbox=self.label.getBBox();*/
            ...
            $feedbackDisabled.css({
                'display': self.parent.engine.feedbackEnabled() ? 'none' : 'block'
            });
            ...
            // Create container:
            var $colorContainer = $('<div id="colorwheel-container"></div>').css({
                'width': '150px',
                'height': '150px'
            });
        ...
        this.move = function(deltax, deltay) {
            this.label.attr({
                "x": this.label.attr("x") + deltax
            });
            this.label.attr({
                "y": this.label.attr("y") + deltay
            });
        };
    }
    ...
    function QuizLabelDrag(parent, label, color) {
      ...
      return;
            ...
            self.label.set.forEach(function(elm) {
                elm.attr({
                    x: elm.attr("x") + (x - self.ox),
                    y: elm.attr("y") + (y - self.oy)
                });
            });
            ...
            for (var i = QuizElementList.elements.length; i--;) {
                ...
                if (QuizElementList.elements[i].uiHotspot.hotspot.margin.isPointInside(x, y)) {
                    ...
                    if (QuizUtil.current_glow === null || QuizUtil.current_glow.parent_id != id) {
                        ...
                        QuizUtil.current_glow = QuizElementList.elements[i].uiHotspot.hotspot.glow({
                            "color": self.color,
                            width: self.label.attr('width'),
                            fill: true,
                            opacity: 1.0
                        });
      ...
      return;
        ...
        this.pointer_up = function(event) {
            ...
            if ((!feedback_enabled && selected_hotspot !== null) || (feedback_enabled && correct_answer)) {
                ...
                // Glue it to the center of the selected hotspot:
                self.set.animate({
                    cx: selected_hotspot.attr("cx"),
                    cy: selected_hotspot.attr("cy"),
                    r: selected_hotspot.attr("r")
                }, 500, "bounce", function() {
            ...
            if (selected_hotspot === null || (feedback_enabled && !correct_answer)) {
                ...
                var bbox = self.label.getBBox();
                self.set.animate({
                    cx: self.label.attr("x") + (bbox.width / 2),
                    cy: self.label.attr("y") + 50,
                    r: self.parent.engine.conf.pointer.radius
                }, 500, "bounce", function() {
                    self.updateVisual(self.pointer.attr("cx"), self.pointer.attr("cy"), false);
                });
    ...
    function QuizHotspot(parent, color, id, x, y, radius) {
        ...
        this.updateHotspotRadius = function(radius) {
            ...
            this.ie_fix.attr({
                x: this.ie_fix.attr('x') - diff,
                y: this.ie_fix.attr('y') - diff,
                width: radius * 2,
                height: radius * 2
            });
        };
        ...
        this.getCenter = function() {
            if (this.hotspot) {
                return {
                    x: this.hotspot.attr('cx'),
                    y: this.hotspot.attr('cy')
                };
            } else {
                return null;
            }
        ...
        this.move = function(deltax, deltay) {
            this.hotspot.attr({
                "cx": this.hotspot.attr("cx") + deltax
            });
            this.hotspot.attr({
                "cy": this.hotspot.attr("cy") + deltay
            });
        };
    }
